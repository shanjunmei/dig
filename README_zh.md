...
> **当前版本**：v1.0.24 — 完整发版说明见 [CHANGELOG.md](./CHANGELOG.md)。
...
| `-cache` | `false` | 将提取出的 IR 缓存到磁盘；命中缓存时跳过提取步骤（类型检查 `packages.Load` 仍每次执行，未被缓存） |
...
| 最新版本 | v1.0.24 | v0.7.0（2025 年 8 月，beta） | v1.24.0（2025 年 5 月） |
